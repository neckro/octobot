...
module.exports = class Plugin extends EventEmitter {
  constructor (pluginModule, pluginOptions) {
    ...
    // Prep the commands
    if (!_.isObject(this.commands)) this.commands = {};
    const prefix = _.isString(this.config.prefix) ? this.config.prefix : '';
    _.forEach(this.commands, (cmd, cmdName) => {
      cmd.name = cmdName;
      cmd.trigger = prefix + cmdName + (cmd.no_space ? '' : ' ');
      if (_.isString(cmd.response)) {
        const response = cmd.response;
        cmd.response = function (evt, msg) {
          msg.reply(response);
        };
      }
      if (!_.isFunction(cmd.response)) {
    ...
    this.once('start', function () {
      ...
      Promise.resolve(initPromise)
        ...
        .catch(err => {
          this.dispatch('log:error', `Error starting plugin ${this.name}:`, err);
          this.dispatch('plugin:failure', this.name);
        });
    });
  }
...
const Listeners = {
  'irc:message': function messageListener (evt, irc) {
    ...
    if (!_.some(this.commands, (cmd, cmdName) => {
      if (input.indexOf(cmd.trigger) !== 0) return false;
      // Found a match
      const msg = irc.text.substr(cmd.trigger.length).trim();
      const params = msg.length > 0 ? msg.split(' ') : [];
      irc = _.extend({}, irc, {
        command: cmdName,
        handler: cmd,
        msg,
